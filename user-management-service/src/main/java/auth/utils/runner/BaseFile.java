...
public class BaseFile {
    ...
    public Role createRolePrivileges(String roleName, List<String> privileges, Institution institution) throws Exception {
        Role role = createRole(roleName, institution);
    ...
    private Role createRole(String roleName, Institution institution) throws Exception {
        UUID uuid = institution != null ? institution.getPublicId() : null;
        Optional<Role> optionalRole = roleRepository.findByNameIgnoreCase(roleName);
        if (!optionalRole.isPresent()) {
            Role role = roleService.createRole(new CreateRoleDTO(roleName, roleName + " created", uuid));
            return role;
        }
